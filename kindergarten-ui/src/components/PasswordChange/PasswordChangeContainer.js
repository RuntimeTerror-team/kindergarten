...
=======
import urls from '../../constants/urls';
>>>>>>> c75efb20688e83a87411cd2e0ba3fa2958c544bb
...
class PasswordChangeContainer extends Component {
  ...
  handleChange = (e) => {
    ...
    if (
      !/^(?=.*[a-ząčęėįšųū])(?=.*[A-ZĄČĘĖĮŠŲŪ])(?=.*\d)[a-ząčęėįšųūA-ZĄČĘĖĮŠŲŪ\d]{8,}$/.test(value) &&
      name === "password"
    ) {
  ...
  checkFieldsValidation = () => {
    if (
      this.state.guardianName.trim().length < 3 ||
      this.state.guardianName.trim().length > 20 ||
      /\d/.test(this.state.guardianName)
    ) {
    ...
    if (
      this.state.guardianSurname.trim().length < 3 ||
      this.state.guardianSurname.trim().length > 30 ||
      /\d/.test(this.state.guardianSurname)
    ) {
  ...
  guardiansValidation = (name, surname, personalCode, phone, address, city, postalCode, email) => {
    return (
      ...
      surname.trim().length <= 30 &&
      personalCode.length === 11 &&
      /^[0-9]*$/.test(personalCode) &&
      phone.length === 12 &&
      /^\+?370[0-9]*$/.test(phone) &&
      address.length >= 8 &&
      address.length <= 50 &&
  ...
  handleGuardianSave = (e) => {
    ...
    if (this.state.isDisabled) {
      ...
      if (
        this.checkEmptyGuardianInputs(
          this.state.guardianName,
          this.state.guardianSurname,
          this.state.guardianId,
          this.state.guardianPhone,
          this.state.guardianAddress,
          this.state.guardianCity,
          this.state.guardianPostalCode,
          this.state.guardianEmail
        ) ||
        !this.guardiansValidation(
          this.state.guardianName,
          this.state.guardianSurname,
          this.state.guardianId,
          this.state.guardianPhone,
        ...
        if (
          ...
          axios
            .put(baseUrl + "/api/persons/" + this.state.userPerson.id, guardian)
            .then((res) => {
  ...
  handleSubmit = (e) => {
    ...
    if (
      ...
      this.state.oldPasswordValidation === "" &&
  ...
  doValidation = (password, password2, oldPassword) => {
    if (
      ...
      !/^(?=.*[a-ząčęėįšųū])(?=.*[A-ZĄČĘĖĮŠŲŪ])(?=.*\d)[a-ząčęėįšųūA-ZĄČĘĖĮŠŲŪ\d]{8,}$/.test(password)
    ) {
      this.setState({ passwordValidation: "is-invalid" });
    }
    ...
    if (
      ...
      !/^(?=.*[a-ząčęėįšųū])(?=.*[A-ZĄČĘĖĮŠŲŪ])(?=.*\d)[a-ząčęėįšųūA-ZĄČĘĖĮŠŲŪ\d]{8,}$/.test(password2)
    ) {
      this.setState({ password2Validation: "is-invalid" });
    }
...
<<<<<<< HEAD
      <div className="footerBottom">
        <ApplicationFormHeader userRole="ROLE_GUARDIAN" name={this.state.userName} surname={this.state.userSurname} />
